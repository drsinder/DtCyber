...
static void lp3000Init(u8 unitNo, u8 eqNo, u8 channelNo, int flags)
    ...
    */
    sprintf(tempfname, "LP5xx_C%02o_E%o", channelNo, eqNo);		//drs
...
void lp3000RemovePaper(char *params)
    ...
    */
    sprintf(tempfname, "LP5xx_C%02o_E%o", channelNo, equipmentNo);
	strcpy(fname, printDir);
	strcat(fname, tempfname);
    ...
    */
    ...
    case FcPrintRelease:
        ...
        if (lc->printed)
            {
            fflush (fcb);
			if (autoRemovePaper != 0)
			{
				sprintf(packer, "%i,%i", active3000Device->channel->id, active3000Device->eqNo);
